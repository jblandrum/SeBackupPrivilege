...
namespace bz.OneOEight.SeBackupPrivilege
{
	...
	[Cmdlet(VerbsCommon.Get, "SeRestorePrivilege")]
	public class Get_SeRestorePrivilege : Cmdlet
	{
		protected override void EndProcessing()
		{
			bool retVal = SeBackupPrivilegeUtils.isSeRestorePrivilegeEnabled();
			Console.WriteLine("SeBackupPrivilege is " + (retVal ? "enabled" : "disabled"));
		}
	}

	[Cmdlet(VerbsCommon.Set, "SeRestorePrivilege")]
	public class Set_SeRestorePrivige : Cmdlet
	{
		[Parameter(HelpMessage = "Disable SeBackupPrivilege instead of enabling it")]
		public SwitchParameter Disable;
		protected override void EndProcessing()
		{
			bool retVal = SeBackupPrivilegeUtils.setSeRestorePrivilege(!Disable.ToBool());
		}
	}
	...
	public class Copy_FileSeBackupPrivilege : PSCmdlet
	{
		...
		protected override void EndProcessing() {
			...
			retVal = SeBackupPrivilegeUtils.CopyFileSeBackup(FileUtils.expandPath(cwd,InFile), FileUtils.expandPath(cwd,OutFile), ref bytesCopied, Overwrite.ToBool());
			if(retVal) {
	...
	[Cmdlet(VerbsCommon.Copy, "FileSeRestorePrivilege")]
	public class Copy_FileSeRestorePrivilege : PSCmdlet
	{
		[Parameter(HelpMessage = "Input file name", Position = 1)]
		public string InFile;
		[Parameter(HelpMessage = "Output file name", Position = 2)]
		...
		protected override void EndProcessing()
		{
			...
			bool retVal;
			string cwd = this.SessionState.Path.CurrentFileSystemLocation.Path;
			retVal = SeBackupPrivilegeUtils.CopyFileSeRestore(FileUtils.expandPath(cwd, InFile), FileUtils.expandPath(cwd, OutFile), ref bytesCopied, Overwrite.ToBool());
			if (retVal)
			{
				Console.WriteLine("Copied " + bytesCopied.ToString() + " bytes");
			}
	...
	[Cmdlet(VerbsCommon.Copy, "FileWTFBBQ")]
	public class Copy_FileWTFBBQ : PSCmdlet
	{
		[Parameter(HelpMessage = "Input file name", Position = 1)]
		public string InFile;
		[Parameter(HelpMessage = "Output file name", Position = 2)]
		...
		protected override void EndProcessing()
		{
			...
			bool retVal;
			string cwd = this.SessionState.Path.CurrentFileSystemLocation.Path;
			retVal = SeBackupPrivilegeUtils.CopyFileNeitherDACL(FileUtils.expandPath(cwd, InFile), FileUtils.expandPath(cwd, OutFile), ref bytesCopied, Overwrite.ToBool());
			if (retVal)
			{
				Console.WriteLine("Copied " + bytesCopied.ToString() + " bytes");
			}
